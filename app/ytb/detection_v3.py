...
# Paramètres d'immobilité et blacklist
IMMOBILE_THRESHOLD = 10    # px (zone de tolérance)
IMMOBILE_DURATION = 3.0    # sec (temps avant blacklist)
BLACKLIST_RADIUS = 25      # px (taille de la zone blacklistée)
...
def main():
    ...
    blacklisted_zones = []         # Liste des zones blacklistées
    ...
    while True:
        if not paused:
            ...
            for r in results_shuttle:
                for box in r.boxes:
                    ...
                    if SHUTTLE_CLASS_NAME in label.lower() and conf > CONFIDENCE_SHUTTLE:
                        ...
                        # --- BLACKLIST CHECK ---
                        in_blacklist = False
                        for bx, by, br in blacklisted_zones:
                            if np.hypot(cx - bx, cy - by) <= br:
                                in_blacklist = True
                                break
                        if in_blacklist:
                            continue  # Ignore toute détection dans une zone blacklistée
                        ...
                        if last_shuttle_pos is not None and \
                            ...
                            if immobile_since is None:
                                ...
                                # On ajoute à la blacklist si pas déjà dedans
                                already_in = False
                                for bx, by, br in blacklisted_zones:
                                    if np.hypot(bx - cx, by - cy) <= br:
                                        already_in = True
                                        break
                                if not already_in:
                                    blacklisted_zones.append((cx, cy, BLACKLIST_RADIUS))
            ...
            # === Affichage ===
            # Shuttlecock (vert) : seulement s'il n'est pas ignoré ou blacklisté
            if shuttle_center is not None:
            ...
            # Affichage des zones blacklistées (rouge)
            for bx, by, br in blacklisted_zones:
                cv2.circle(frame_proc, (bx, by), br, (0,0,255), 2)
                cv2.putText(frame_proc, "BLACKLIST", (bx-br, by-br), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 2)
